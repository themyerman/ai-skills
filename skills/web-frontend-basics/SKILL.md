...
description: >-
  ...
  **web-accessibility** (see Related). Semantic HTML, forms, small JS, fetch+JSON
  ...
  python-scripts-and-services flask-serving; visuals: web-layout-css; a11y:
  web-accessibility.
---
...
**Browser-side** building blocks for **server-rendered** or **light-JS** internal tools: **HTML** that means something, **forms** that fail accessibly, and **JavaScript** that stays small and testable. For **Flask routes, Jinja, auth, headers,** and **`static/`** layout, use **[`python-scripts-and-services` / `flask-serving.md`](../python-scripts-and-services/flask-serving.md)**. For **layout and CSS**, see **[web-layout-css](../web-layout-css/SKILL.md)**. For **WCAG and keyboard**, see **[web-accessibility](../web-accessibility/SKILL.md)**.
...
- **Python** HTTP clients, Jira, secrets → **python-scripts-and-services** / **security.md**.
...
**Server-side** (routes, `static/`, auth, headers): [**flask-serving**](../python-scripts-and-services/flask-serving.md) in **python-scripts-and-services**. Repo-level chart: [`SKILLS.md`](../../SKILLS.md#web-ui-default-reading-order) in **ai-skills**.

## Related

- **web-layout-css** — flex/grid, responsive, component-ish patterns (step 2)  
- **web-accessibility** — keyboard, ARIA, focus, contrast (step 3)  
- **python-scripts-and-services** — **flask-serving.md**, **security.md**

## Source

Authored for **ai-skills**; **not** a slice of **`.claude/CLAUDE.md`**. **Pair** with [flask-serving](../python-scripts-and-services/flask-serving.md) and your project’s `templates/`, `static/`.
